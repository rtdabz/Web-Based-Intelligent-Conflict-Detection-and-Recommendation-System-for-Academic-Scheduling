...
export default function WorkflowGuideButton({ guideId, id, className = "" }: WorkflowGuideButtonProps) {
  ...
  return (
    <button
      ...
      className={`fixed ${profileMenuOpen ? "bottom-4 top-auto" : "top-[5.5rem] bottom-auto"} right-3 z-30 inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#C9952A]/60 bg-white/95 text-xs font-bold text-[#4e0a10] shadow-lg transition-all hover:-translate-y-0.5 hover:border-[#C9952A] hover:bg-[#fff8e8] hover:shadow-xl sm:right-6 ${className}`}
    >
