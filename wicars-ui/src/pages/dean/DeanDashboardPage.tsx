...
export default function DeanDashboardPage() {
  ...
  const deptFaculties = useMemo(() => {
    if (!user?.department_id) return faculties;
  ...
  const visibleSectionIds = useMemo(() => new Set(deptSections.map(s => s.id)), [deptSections]);
  ...
  const calendarAvailableRooms = useMemo(() => {
    return Math.max(0, rooms.length - calendarRoomsInUse);
  }, [rooms, calendarRoomsInUse]);
  ...
  const overallStats = useMemo(() => {
    ...
    return { approvedCount, pendingCount, draftCount, rejectedCount };
  }, [visibleSchedules]);
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 items-stretch">
          ...
          <div className="xl:col-span-6 space-y-4 flex flex-col h-full">
            ...
            <div className={`${
              ...
              <div className="mt-2 flex-1 overflow-auto border border-gray-200 rounded-xl max-h-[600px] relative bg-white select-none">
                <div className="min-w-[850px] flex flex-row">
                  {/* Time Axis Column */}
                  <div className="w-20 shrink-0 border-r border-gray-200 bg-gray-50/70 select-none">
              ...
              <div className="border-t border-gray-100 pt-3 mt-3 flex items-center justify-between text-[11px] text-gray-400">
                <span>Dean Academic Schedule Matrix</span>
                <button
                  onClick={() => navigate('/dean/schedules')}
          ...
          <div className="xl:col-span-6 space-y-4 flex flex-col h-full font-sans">
            ...
            <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm flex-1 flex flex-col justify-between">
              <div>
                ...
                <div className="space-y-2">
                  {(yearLevels && yearLevels.length > 0 ? yearLevels : [
                    { id: 1, label: '1st Year', totalSections: deptSections.filter(s => s.section_name.includes('1')).length || 2, scheduledSections: 2, isComplete: true, percent: 100 },
                    { id: 2, label: '2nd Year', totalSections: deptSections.filter(s => s.section_name.includes('2')).length || 2, scheduledSections: 2, isComplete: true, percent: 100 },
                    { id: 3, label: '3rd Year', totalSections: deptSections.filter(s => s.section_name.includes('3')).length || 2, scheduledSections: 1, isComplete: false, percent: 50 },
                    { id: 4, label: '4th Year', totalSections: deptSections.filter(s => s.section_name.includes('4')).length || 2, scheduledSections: 0, isComplete: false, percent: 0 },
                  ]).map((lvl: any) => {
              ...
              <div className="border-t border-gray-100 pt-3 mt-3 flex items-center justify-between text-[11px] text-gray-400">
                <span>Department Sections: {deptSections.length}</span>
                <button
                  onClick={() => navigate('/dean/schedules')}
                  className="font-bold text-[#5A1220] hover:underline cursor-pointer"
                >
