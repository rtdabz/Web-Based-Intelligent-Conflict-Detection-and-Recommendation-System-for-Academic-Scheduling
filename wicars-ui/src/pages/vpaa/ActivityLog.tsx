...
export default function ActivityLog() {
  ...
  const [filters, setFilters] = useState({ search: '', category: '', department_id: '', semester_id: '', from: '', to: '' });
  ...
  const clearFilters = () => {
    const empty = { search: '', category: '', department_id: '', semester_id: '', from: '', to: '' };
    setFilters(empty);
    setApplied(empty);
    setPage(1);
  };
    ...
    <div id="activity-log-page" className="space-y-5">
      ...
      <form onSubmit={applyFilters} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
          ...
          <select aria-label="Semester" value={filters.semester_id} onChange={event => setFilters(current => ({ ...current, semester_id: event.target.value }))} className="rounded-lg border border-gray-300 px-3 py-2 text-sm">
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onMouseDown={event => { if (event.target === event.currentTarget) setSelected(null); }}>
        <div role="dialog" aria-modal="true" aria-labelledby="activity-title" className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
          ...
          <div className="space-y-5 p-5 text-sm">
            <dl className="grid gap-4 sm:grid-cols-2"><div><dt className="text-xs font-bold uppercase text-gray-500">Actor</dt><dd className="mt-1 text-gray-900">{selected.actor?.name || 'System'} {selected.actor && <span className="text-gray-500">({selected.actor.username})</span>}</dd></div><div><dt className="text-xs font-bold uppercase text-gray-500">Role</dt><dd className="mt-1 capitalize text-gray-900">{selected.actor?.role || 'System'}</dd></div><div><dt className="text-xs font-bold uppercase text-gray-500">Department</dt><dd className="mt-1 text-gray-900">{selected.department_id ? departmentMap.get(selected.department_id)?.department_name || `#${selected.department_id}` : 'Institution-wide'}</dd></div><div><dt className="text-xs font-bold uppercase text-gray-500">Semester</dt><dd className="mt-1 text-gray-900">{selected.semester_id ? `${semesterMap.get(selected.semester_id)?.academic_year || ''} ${semesterMap.get(selected.semester_id)?.semester || `#${selected.semester_id}`}` : 'Not applicable'}</dd></div><div><dt className="text-xs font-bold uppercase text-gray-500">Target</dt><dd className="mt-1 text-gray-900">{formatLabel(selected.target.type)} {selected.target.id ? `#${selected.target.id}` : ''}</dd></div><div><dt className="text-xs font-bold uppercase text-gray-500">Event ID</dt><dd className="mt-1 font-mono text-xs text-gray-700">{selected.id}</dd></div></dl>
