...
export default function ProgramHeadDashboardPage() {
  ...
  // Timetable Calendar Filters and state for Program Head's Department
  ...
  // Calendar filtered schedules: scoped exclusively to program head's own department
  ...
  const calendarAvailableRooms = useMemo(() => {
    return Math.max(0, 15 - calendarRoomsInUse);
  }, [calendarRoomsInUse]);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {(() => {
              const timetableElement = (
                ...
                >
                  {/* Header + Actions Row */}
                  <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 border-b border-gray-150 pb-4 flex-shrink-0">
                  ...
                  {/* Summary Metrics Row */}
                  <div className={`${isFullscreen ? "flex flex-wrap items-center gap-4 mt-3 mb-2 pb-2.5 border-b border-gray-150 flex-shrink-0" : "grid grid-cols-1 md:grid-cols-3 gap-4 mt-4"}`}>
                    {/* Total Classes */}
                  ...
                  <div className={`mt-4 ${isFullscreen ? 'flex-1 min-h-0 overflow-hidden flex flex-col' : ''}`}>
                    <div className="overflow-x-auto overflow-y-auto rounded-2xl border border-gray-200 shadow-inner flex-1 flex flex-col">
